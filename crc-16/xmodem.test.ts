...
Deno.test('table', () => {
	assertEquals(CRC_16_XMODEM.table(), table);
});

Deno.test('123456789', () => {
	let c = CRC_16_XMODEM.init();
	c = CRC_16_XMODEM.update(c, new TextEncoder().encode('123456789'));
	c = CRC_16_XMODEM.finalize(c);
	assertEquals(c, 0x31c3);
});
