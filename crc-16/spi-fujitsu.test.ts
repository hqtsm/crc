...
Deno.test('table', () => {
	assertEquals(CRC_16_SPI_FUJITSU.table(), table);
});

Deno.test('123456789', () => {
	let c = CRC_16_SPI_FUJITSU.init();
	c = CRC_16_SPI_FUJITSU.update(c, new TextEncoder().encode('123456789'));
	c = CRC_16_SPI_FUJITSU.finalize(c);
	assertEquals(c, 0xe5cc);
});
