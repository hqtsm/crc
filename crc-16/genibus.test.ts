...
Deno.test('table', () => {
	assertEquals(CRC_16_GENIBUS.table(), table);
});

Deno.test('123456789', () => {
	let c = CRC_16_GENIBUS.init();
	c = CRC_16_GENIBUS.update(c, new TextEncoder().encode('123456789'));
	c = CRC_16_GENIBUS.finalize(c);
	assertEquals(c, 0xd64e);
});
