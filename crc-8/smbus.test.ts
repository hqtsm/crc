...
import CRC_8_SMBUS from './smbus.ts';
...
Deno.test('table', () => {
	assertEquals(CRC_8_SMBUS.table(), table);
});

Deno.test('123456789', () => {
	let c = CRC_8_SMBUS.init();
	c = CRC_8_SMBUS.update(c, new TextEncoder().encode('123456789'));
	c = CRC_8_SMBUS.finalize(c);
	assertEquals(c, 0xf4);
});
